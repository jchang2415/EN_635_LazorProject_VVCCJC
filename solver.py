...
def check_solution(hit_paths, target_points):
    ...
    for point in target_points:
        ...
        # If a target is missed, stop checking early, as the solution already cannot be valid
...
def generate_block_combinations(board):
    ...
    # Use a combination method to generate all possible COMBOS of slot placements (since order doesn't matter between blocks of same type)
    ...
    for a_slots in combinations(slots_idx, movable_counts.get('A', 0)):
        ...
        # For the remaining open slots, generate all possible placement of B blocks
        for b_slots in combinations(remaining_after_a, movable_counts.get('B', 0)):

            # Repeat for blocks of type C
            remaining_after_b = [i for i in remaining_after_a if i not in b_slots]
            for c_slots in combinations(remaining_after_b, movable_counts.get('C', 0)):
                
                # Generate the placement dictionary mapping blocks to the found slots
                placement = {}
                for i, slot in enumerate(open_slots):
                    
                    # If the index has been set aside for an A block, fill it in as such
                    if i in a_slots:
                        placement[slot] = 'A'
